		...
		$(document).ready(function() {
			...
			$('#uploadButton').on('click', async (e) => {
				...
				if (monthId && year && categoryId) {
					...
					result = await $.ajax(`/bankjatim/upload/check_file.php?year=${year}&month_id=${monthId}&category_id=${categoryId}&work_unit=4`)
				} else if(year && categoryId) {
					result = await $.ajax(`/bankjatim/upload/check_file.php?year=${year}&category_id=${categoryId}&work_unit=4`)
				} else {
